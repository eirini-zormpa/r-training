...
Find out the current content of the object `area_acres`.
...
Because we have not rerun the code `area_acres <- 2.47*area_hectares` after assigning the new value of `area_hectares` the value of `area_acres` is still `6.175`.
...
Create a third variable `r_area` and give it a value based on the current values of `r_length` and `r_width.`
Show that changing the values of either `r_length` or `r_width` does not affect the value of `r_area.`
